...
if [[ $# -lt 1 ]]; then
  if [[ -z "${SLURM_ARRAY_TASK_ID}" ]]; then
    ...
    TRAIN_ARGS=($(sed -n ${SLURM_ARRAY_TASK_ID}p ud-2.3.txt))
  fi
else
  TRAIN_ARGS=("$@")
fi
...
TREEBANK_DIR=$(readlink -f ${TRAIN_ARGS[0]})  # Subdirectory of ../data/ud-treebanks-v2.3/
...
mkdir -p ../word_vectors ../converted/ud-treebanks-v2.3 models/ud-2.3
...
CONVERTED_TRAIN_DIR=../converted/ud-treebanks-v2.3/${TREEBANK_DIR_BASENAME}/train
CONVERTED_DEV_DIR=../converted/ud-treebanks-v2.3/${TREEBANK_DIR_BASENAME}/dev
...
tupa --verbose=1 -W --seed=$RANDOM ${DYNET_FLAGS} --max-length=300 \
  ...
  -m "models/ud-2.3/$MODEL_NAME-$SUFFIX" ${TRAIN_ARGS[@]}
